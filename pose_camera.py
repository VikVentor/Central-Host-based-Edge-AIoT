...
import socket  # Added for communication with ESP32
import time
import numpy as np
import svgwrite
import gstreamer
...
ESP32_IP = "192.168.74.166"  # Replace with the ESP32's IP address
ESP32_PORT = 12345          # Port number to communicate with ESP32

esp32_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
esp32_socket.connect((ESP32_IP, ESP32_PORT))  # Connect to ESP32
...
def draw_pose(dwg, pose, src_size, inference_box, color='yellow', threshold=0.2):
    ...
    xys = {}
    top_left_threshold = (src_size[0] * 0.25, src_size[1] * 0.25)  # Define top-left area
    top_right_threshold = (src_size[0] * 0.75, src_size[1] * 0.25)  # Define top-right area
    ...
    for label, keypoint in pose.keypoints.items():
        ...
        # Check for top-left and top-right positions
        if label == KeypointType.LEFT_WRIST and kp_x < top_left_threshold[0] and kp_y < top_left_threshold[1]:
            print("left")
            esp32_socket.sendall(b"1")  # Send "1" to ESP32
        if label == KeypointType.RIGHT_WRIST and kp_x > top_right_threshold[0] and kp_y < top_right_threshold[1]:
            print("right")
            esp32_socket.sendall(b"2")  # Send "2" to ESP32
...
if __name__ == '__main__':
    try:
        ...
        esp32_socket.close()  # Ensure socket is closed on exit
